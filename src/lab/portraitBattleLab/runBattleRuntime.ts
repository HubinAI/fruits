 ...
 *      （`runModifiers.composeRunWeaponDef`），再重映射 `defId`；能力侧（动能爆发 / 压制射击）
...
export class RunBattleRuntime {
  ...
  readonly build: readonly RunModifierId[];
  /**
   * 本场战斗的 Run 能力（动能爆发 / 压制射击）—— 事件驱动，见 `runBuildAbilities.ts`。
    ...
    this.abilities = new RunBuildAbilities(
      {
         ...
         * 省略 / `null` → 作用在该车自身位置（**当前两项能力都传真实作用点**：动能爆发与
         * 压制射击都用命中点 `damage.contactPoint` → 这条兜底分支只是端口的通用语义，
         * 本局不会被走到）。
