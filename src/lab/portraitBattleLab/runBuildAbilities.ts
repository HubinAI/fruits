 ...
 *
 * ⚠️ **PRP-BUILD-01-CLOSEOUT-AND-FREEZE**：本文件曾同时实现「快速装填的专属第二层」，
 *    三条尝试（`反冲蓄能` → `强力后坐` → `压制射击`）**全部真人未通过**，已正式废弃并整条删除。
 *    删除时按 Queue 要求区分：**只删这些失败实验专属的东西**；
 *    同时服务「动能爆发」的共享接缝（`weaponFire` 方向记录 / `damage` 命中入口 /
 *    `RunAbilityPorts` / `flush()` 步边界施加）**全部保留**，未因清理误删。
 */
...
export class RunBuildAbilities {
  ...
  private onEvent(ev: BattleEvent): void {
    ...
    if (ev.type === 'weaponFire') {
      ...
      //    动能爆发需要「这一发的方向」作为击退方向。
      //    ⚠️ PRP-BUILD-01-CLOSEOUT-AND-FREEZE：开火**不触发**任何自车冲量 ——
      //       曾经在这里的「一炮一后坐」随 `strongRecoil` 整条删除，不再恢复。
