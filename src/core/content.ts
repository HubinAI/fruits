...
const wedgeShovel: FunctionalPartDef = {
  ...
  // 本地 x∈[-26, 68]（长 94）、y∈[-18, 2]（高 20，低矮）；offset 把楔形
  // 放到挂点前方且贴近地面（offset.y 大 = 向下）。
  // Q11-A-R1（最小几何修复，根因唯一——诊断实测：旧楔尖 y≈674 高于对手
  // 底盘下缘 680，楔铲藏在自车底盘内，第一次接触只是侧面挤压（14s 无姿态
  // 改变，17s 撞墙才掀翻））：
  // 1. offset.y 22→38：楔尖（世界 y≈690）深插对手底盘空隙（boxBody 底盘
  //    下缘≈680 → 地面 700，20px 空隙）——第一次接触楔形前端即进入对手
  //    底部，对手底盘被垫起/明显抬头（实测 0.7s 内抬 10px + 抬头 10°）。
  // 2. 楔尖 x 38→78（楔形加长 40px）：楔尖够到对手前轮下方（轮子左缘
  //    恰与左壁平齐）；旧 64px 楔尖停在轮子左缘外 ~0.2px（两车驱动相抵
  //    锁定，14s 无法楔入）。
  collider: {
    shape: 'polygon',
    vertices: [
      { x: 78, y: -2 }, // 前端楔尖（低，贴近地面，伸至对手前轮下方）
      { x: -26, y: -18 }, // 后端坡顶（高，顶起对手底盘）
      { x: -26, y: 2 }, // 后端底面
      { x: 78, y: 2 }, // 前端底面
    ],
    offset: { x: 24, y: 38 },
