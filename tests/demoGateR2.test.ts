...
describe('F-DEMO-GATE-R2｜手机演示版本技术 Gate', () => {
  ...
  it('G2. F-DEMO-FLOW-GATE-R3：真实构建 E2E 门禁替代字符串 Gate（删除 screenToLayoutPoint 检查）', () => {
    ...
    // F-DEMO-VISUAL-GATE-R4｜Must#6：E2E 探针隔离——window.__h/__probe 仅存在于专用 E2E 构建
    // （vite.e2e.config.ts define __WX_DEBUG__=true）；正式 pages define 显式 false →
    // 编译期折叠（生产 bundle 零探针；运行时 undefined）。原 __E2E_PROBE__ 已弃用。
    const e2eVite = readFileSync('vite.e2e.config.ts', 'utf-8');
    const pagesVite = readFileSync('vite.pages.config.ts', 'utf-8');
    expect(e2eVite, 'E2E 专用构建注入 __WX_DEBUG__=true').toContain("__WX_DEBUG__: 'true'");
    expect(pagesVite, '正式 Pages 显式 __WX_DEBUG__=false（编译期折叠）').toContain("__WX_DEBUG__: 'false'");
    ...
    expect(hostSrc, 'host 探针受 __WX_DEBUG__ 守卫').toMatch(/typeof __WX_DEBUG__ !== 'undefined' && __WX_DEBUG__/);
