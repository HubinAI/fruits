...
/**
 * 预置**五份**一次性迁移的标记 ⇒ 本文件只观察 Movement 维度本身。
 *
 ...
 * ⚠️ 第五份（`markR5ContentPoolSeed()`）是 PRODUCT-LOOP-R5-BASIC-CONTENT-POOL-R1 追加的：
 *    内容池种子同样会落**自己的** key，并在首入时给全部 `OFFICIAL_PARTS` 补 ★1 ×1
 *    ⇒ 不预置的话 MO-11 的「本次挂载不新增任何 key」会红（多出 R5 的 key）。
 *    理由与第三 / 第四份逐条同源：**隔离变量，不是放宽**；那份种子的契约由
 *    `tests/productContentPoolSeed.test.ts` 单独测。
 */
...
describe('A. 新账号默认 Movement 拥有状态（必改 2）', () => {
  it('MO-01 全新账号：缺省轮**恒默认拥有**，其余三档未拥有，不变式 legal=真', () => {
    isolateMigrations();
    // ⚠️ PRODUCT-LOOP-R4-BODY-CANONICAL-AND-GARAGE-MVP：`isolateMigrations()` 现在预置**五份**
    //    一次性迁移标记（R2 onboarding / R2 reseed / R3 Movement seed / R4 Body seed /
    //    R5 内容池 seed）。本断言只是夹具前提（「磁盘上只有迁移标记，没有别的」）
    //    ⇒ 数字随预置份数同步，**闭集语义一字未改**：多出任何别的 key 这一条照样红。
    expect(store.length, '夹具前提：磁盘上只有五份迁移标记，没有别的').toBe(5);
