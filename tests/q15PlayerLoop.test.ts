 ...
 *
 * 覆盖：
 * 1. 对手池（OPPONENT_POOL）：49 套、彼此不同、全部合法 Build、6 种 Body
 *    （西瓜/香蕉/菠萝/椰子 + R1 新增 heavyBox/tallBody）
 *    + 当前正式 PART_OPTIONS、不含 wedge/ramHead/lifter/旋锤 等 HOLD 内容；
 *    （Q16：6→24 套铺量；Q19：24→36 套，新增菠萝/椰子，覆盖 6 类战斗身份；
 *     F-CONTENT-OPPONENT-BUILD-POOL-R1：36→49 套，新增 13 套模板，重型/高重心首次进池）
...
const ALLOWED_BODIES = new Set(['watermelonBody', 'bananaBody', 'pineappleBody', 'coconutBody', 'heavyBox', 'tallBody']);
...
describe('Q15 对手池', () => {
  it('1. 共 49 套对手配置（Q16 + Q19 + R1 铺量）', () => {
    expect(OPPONENT_POOL.length).toBe(49);
  });

  it('2. 49 套配置彼此明显不同（无重复）', () => {
  ...
  it('4. 只使用 6 种正式 Body（西瓜 / 香蕉 / 菠萝 / 椰子 / 重箱 / 高重心）', () => {
    for (const d of OPPONENT_POOL) {
...
describe('Q16 对手池铺量（24 套）', () => {
  ...
  it('5. 6 种 Body 均有实际覆盖（西瓜/香蕉 ≥10，菠萝/椰子 ≥4，重箱/高重心 ≥4，总计 49）', () => {
    ...
    const hb = OPPONENT_POOL.filter((d) => d.bodyDefId === 'heavyBox').length;
    const tb = OPPONENT_POOL.filter((d) => d.bodyDefId === 'tallBody').length;
    ...
    expect(tb).toBeGreaterThanOrEqual(4);
    expect(wm + bn + pa + co + hb + tb).toBe(OPPONENT_POOL.length);
  });
