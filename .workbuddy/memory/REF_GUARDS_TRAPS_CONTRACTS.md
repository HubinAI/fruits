...
## §14 R2 验证起点的**版本化一次性 reseed**（PRODUCT-LOOP-R2-VALIDATION-STATE-RESEED-R1 固化契约；改动一次性迁移 / 成长读入口前必读）

### §14a 症状与真实成因

真人已用 `r2Onboarding` 完整验过一轮（`4/5 → 领一件 → 5/5 → 5合1 → ★2 → 装备 ★2`）⇒ 这份存档**再也回不到起点**：
- `cannon ★2` 已存在**且装备着**（上一轮验证的产物）；
- 新领的 `cannon ★1` 只有 **1/5**；
- 旧 onboarding 标记**已消费** ⇒ 不会再补。

⚠️ **关键：旧 `r2Onboarding` 结构上不可能救回来** —— 它的判据 ② 是「存在 ★≥2 的 Weapon 就**一个字节都不动**」，
那正是它当初的**保护逻辑**。所以「把旧标记 +1」是**死路**（Queue 必改 1 明令不许复用已消费的旧 key）。

### §14b 为什么必须**新 key / 新模块**（而不是把 `R2_ONBOARDING_VERSION` +1）

两件事**写语义相反**，混在一处会让两边的不变量都无法审计：
- `r2Onboarding` = **只增不减**（把「一个字节都不改」写进注释、并被 `PG-*` 用例钉住）；
- `r2Reseed`    = **必须删除** `cannon` 的 ★≥2（这正是本轮目标）。

且旧标记的语义（「这个账号的验证起点已安排过」）**仍然为真** —— 我们只是新增了**另一个**事实。
⇒ 新 key `strongfruit.r2Reseed.v1` + 新模块 `src/product/r2Reseed.ts`。

### §14c 五条判据（**同时**成立才执行）+ 为什么 ②③ 两条都要

① `isR2ReseedDone()` 为假（**本模块**的标记不在盘上）· ② `isR2OnboardingDone()` 为真（= 打开过产品首页）·
③ `hasPrototypeClaim()` 为真（`strongfruit.profileClaims.v1` 非空）· ④ `grownCannonCount(inv) > 0`（= 真合成过）·
⑤ `cannon ★1 < 4`（= 起点真被消耗）。

⚠️ **库存 key 是旧横屏游戏与竖屏产品共用的**，而旧横屏的 `fuseSameStar` **同样**能造出 `cannon ★2`
⇒ 只看 ④⑤ 会误伤「只在旧横屏合成过」的老玩家。因此要求「这个账号走过**产品**验证链」，两条独立证据：
- ② 只要 `openGrowthSession` 跑过一次就会落；
- ③ `saveClaimLedger` 在整个 `src/` 里**只有一处**调用点（`playerProfile.claimRunReward` 的成功分支）
  ⇒「账本非空」= 至少真领过一次奖，而那正是「4 件 + 1 件领奖 = 5 件才能合成 ★2」的**结构性前提**。

⚠️ **② 单独用不够**：只在旧横屏合成过 ★2 的老玩家**第一次**打开产品首页时 ② 不成立
（`markR2Onboarding()` 要等本次挂载**末尾**才落盘），但**第二次**挂载标记已在盘上 ⇒ 只靠 ② 会在第二次挂载命中他。
③ 才是那个真正独立、且**只可能由产品链路产生**的证据。
⚠️ **已知边界（如实披露，不是静默假设）**：同时满足「旧横屏合成过 ★2」+「打开过产品首页」+「在原型里领过
至少一次奖」的账号也会被清掉 `cannon ★2`。该类账号已是 Queue 描述的原型验证账号，故**接受**；
更细的区分在当前存档里**没有信号**。

### §14d 落盘顺序 + 装备失败必须**零副作用**

`planR2Reseed`（**纯判定，一个字节都不写**）与 `applyR2Reseed` **分开**是刻意的（与 `planR2Onboarding` 同理）：
调用方要把「落盘」排在「打标记」**之前**；塞进一个函数里，那个顺序在结构上就无法表达。

执行四步：① `addPart(cannon, ★1, 补到 4)`（**仅内存**）→ ② `equipWeapon('cannon', draft, inv, 1)`
（唯一写 Build 的入口，过正式 `validateSnapshot`；⚠️ 它**只读传进去的那份 `inv`、不读盘** ⇒ ① 必须先于它）→
③ `consume` 掉 ★2..★5（仅内存）→ ④ `saveInventory(inv)`：**唯一一次落盘**，`★1 = 4` 与「无 ★≥2」**原子生效**。

⚠️ **装备被拒 ⇒ 整体不做**（而不是「先把 ★1 补到 4 再说」）：只补数量会让账号停在一个**谁也说不清**的中间态，
而且判据 ⑤ 会立刻翻成 false ⇒ **永远不再重试**。做法 = 失败分支用 `consume` **原样回滚** ① 的内存改动；
`equipWeapon` 的校验全部发生在 `persistPlayerBuild` **之前** ⇒ ② 失败时**零落盘**
⇒ 调用方的 `inv` / `draft` 与调用前**逐字节相同**（下次挂载可重试）。
这与 `playerGrowth.fuseStack` 的 `equip-failed` 回滚是**同一条纪律**，不是新发明的规则。

### §14e ⚠️ `homePage` 必须消费 `growth.draft`（否则屏上一个样、盘上另一个样）

reseed **会改写 Build**（把主武器槽换成 `cannon ★1`）⇒ `openGrowthSession` 必须返回一份**规范化后的 `draft`**。
`homePage` 若继续用**会话前**那份 draft 渲染，就会出现「屏幕显示 ★2、磁盘是 ★1」的**两源分叉**。
⇒ `const growth = openGrowthSession(draft); draft = growth.draft;`（源码守卫钉住这一行）。

### §14f 陷阱（本轮实测，会毁掉你的夹具 / 断言）

1. ⚠️ **★1 = 缺省 ⇒ `functionalStars` 整个键都必须不存在**。`equipWeapon(star = 1)` 会把
   `functionalStars[WEAPON_SLOT]` **删掉**（`buildEditorModel`：「缺省 = 全 ★1」）⇒ 断言里读
   `draft.functionalStars[WEAPON_SLOT]` 会**直接抛**「Cannot read properties of undefined」。
   要断的是 `functionalStars === undefined`。
2. ⚠️ **落标记的判据是「决策」（`decided`）而不是「动作」（`applied`）** —— 这一条是本轮门禁
   **用一次真实丢档换来的**（`e2e:product-reward` 的 K8 / H2 / H3 同时红，磁盘读数 `★1=4 ★2=0`）：
   首入时还没领过奖（`no-claim`）若不落标记，玩家随后自己走完 `领奖 → 5/5 → 合成 ★2`，
   下一次挂载时五条判据**全部成立** ⇒ 他**自己刚合出来的** ★2 会被当成「上一轮的产物」清掉。
   ⇒ **四个「一个字节都不动」的出口（`not-prototype` / `no-claim` / `not-consumed` / `start-intact`）
   一律 `decided = true`**（= 落标记）；**唯一例外**是 `equip-failed`（可重试的瞬时失败，
   落标记会把账号永久钉死在「起点已消费」的形态上 ⇒ 验收 ① 永远达不成）。
   代价（如实披露）：只有 `equip-failed` 这一种失败会每次挂载重判一次 —— 判据全是**纯读**
   （五次 `getCount` + 三个标记读数），**零写入、零无谓持久化**。
3. ⚠️ **负控制不能省**：把 `planR2Reseed` 末行改成 `return keep('not-consumed')` 后，单测 **10 failed / 9 passed**、
   E2E **6/16 PASS**（R2 / R2b / R2c / R2d / R2g / R3 / R4 / R4d / R4e / R9 全红）⇒ 证明守卫**不是空转**。
   **回退后必须 grep 全仓关键词确认零残留**（本轮用 `NEGCTL`），不能只信「已改回」。
4. ⚠️ **负控制要按「新守卫」再单独做一次**（第 3 条只证明了「判定整块不生效」会被发现，
   证明不了「判定生效了、但标记时机错」会被发现）。把调用方改回旧语义
   `if (reseed.applied) markR2Reseed();` 后，单测 **7 failed / 13 passed** —— 恰好是钉住这条语义的
   7 条：`VR-06` / `VR-06b` / `VR-08` / `VR-09` / `VR-10` / `VR-11` / `VR-15`（关键词 `NEGCTL-B`）。

### §14g 判定时机 = 「新版本首入」，而且**只判一次**（本 Queue 唯一一个真 bug 的修复）

**症状**（`e2e:product-reward` 门禁抓出，与本文件夹具无关）：新账号首入 → 自己打一局 → 领奖 →
合成得到 ★2 → 回首页 ⇒ ★2 被清、★1 被倒补回 4。

**根因**：`planR2Reseed` 的五条判据读的是**当下的库存**，而库存**会被玩家自己改变**
⇒ 「首入判定」被写成了「每次挂载都判」的**连续判定**。判据 ③（领奖账本非空）被当成了
「只要是产品账号就一直为真」的稳定事实，实际它在玩家第一次领奖的瞬间才**由假变真**。

**修法（结构性，不是打补丁）**：给 `R2ReseedPlan` 加 `decided` ——
**「新版本首入的判定结论是否已经做出」**（与 `R2OnboardingPlan.needsMark` 同一形状、同一纪律）。
调用方据此落标记：`if (reseed.decided) markR2Reseed();`。此后任何挂载只看标记、不再重判。

**为什么这是「正确性要求」而不是「优化」**：判据含**单调变化**的库存读数，
`already-marked` 是唯一能把它钉成一次性的东西；不落标记时这个迁移的语义会**随玩家行为漂移**。

⚠️ **两个必然的副作用（都要认，别当 bug）**：
   - 全新账号**第一次**打开首页时 reason 是 **`not-prototype`**（旧 onboarding 标记要等本次挂载
     末尾才落）—— 这是判据顺序决定的，断言要按它写；
